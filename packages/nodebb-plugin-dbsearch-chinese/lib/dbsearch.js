...
search.filterSearchQuery = async function (data) {
	...
	if (data.hasOwnProperty('content')) {
		query.content = tokenizer.forQuery(data.content);
	}
...
async function topicsSave(topics) {
	...
	let data = topics.map((topicData) => {
		const indexData = {};
		if (topicData.title) {
			indexData.content = tokenizer.forIndex(topicData.title);
		}
		if (topicData.cid) {
...
async function postsSave(posts) {
	...
	let data = posts.map((postData) => {
		const indexData = {};
		if (postData.content) {
			indexData.content = tokenizer.forIndex(postData.content);
		}
		if (postData.cid) {
...
async function messagesSave(msgs) {
	...
	let data = msgs.map((msgData) => {
		const indexData = {};
		if (msgData.content) {
			indexData.content = tokenizer.forIndex(msgData.content);
		}
		if (msgData.roomId) {
...
async function renderAdmin(req, res) {
	...
	results.plugin.title = 'DB Search Chinese';
